	...
	public class ControllerInput {
		...
		public virtual void UpdateState() {
			if(Time.frameCount!=m_PrevFrameCount){
                ...
                //               
                //@EDIT:
                //  1,To Solve The Controller TouchPad Swipe/Dpad Event not Exec issue
                //  2,Solve Hmd Input Value Change issue
                //Original----------------------------------------
                //if (XDevicePlugin.DeviceVersion != XDevicePlugin.XIM_DK4_DIS01)
                //{
                //TouchpadToDpad();
                //TouchpadToSwipe();
                //}
                //Original----------------------------------------
                //Now----------------------------------------               
                //Only Controller Compute the Touchpad Event,Skip Hmd Input
		...
		private static readonly float AXIS_AS_BUTTON_THRESHOLD = 0.3f;
		...
		public virtual void StartHaptics(int strength,int frequency,float duration=0.0f) {
			...
			XDevicePlugin.SendMessage(handle,XDevicePlugin.kMessage_TriggerVibration,
                (int)((strength<=0?20:strength)|(int)((frequency<<16)&0xFFFF0000)),
				(int)(duration*1000)
			);
#if UNITY_ANDROID
			if(duration>0.0f) {
